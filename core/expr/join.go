...
func (s *Selection) Join(
	...
	right := inspect.TableReferenceFromAny(rightAny)
	if right == nil {
		msg := fmt.Sprintf(
	...
	on := inspect.BooleanValueExpressionFromAny(onAny)
	if on == nil {
		msg := fmt.Sprintf(
...
func (s *Selection) doJoin(
	...
	*/
	...
	referreds := lo.Uniq(inspect.ReferredFromBooleanValueExpression(on))
