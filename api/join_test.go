...
func TestJoin(t *testing.T) {
	...
	subq := api.Select(colUserId).As("users_derived")
	...
	tests := []struct {
		...
		},
		{
			...
			q: api.Select(
				...
				users,
				api.Equal(colArticleAuthor, colUserId),
			).Where(
				api.IsNull(colArticleAuthor),
			),
		...
		},
		{
			...
			q: api.Select(
				...
				subq, api.Equal(colUserId, subq.C("id")),
			),
...
func TestJoinPanics(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			q:    func() { api.Select().Join(users, api.Equal(colArticleAuthor, colUserId)) },
		},
		{
			name: "Bad JOIN. Can't Join() against a selection that isn't in the containing SELECT",
			q:    func() { api.Select(articleStates).Join(users, api.Equal(colArticleAuthor, colUserId)) },
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestNestedSetQueries(t *testing.T) {
	...
	joinCond := api.And(
		api.Equal(o1rootid, o2rootid),
		api.Between(o1nestedleft, o2nestedleft, o2nestedright),
	)
	q := api.Select(o1id).Join(o2, joinCond)
	q.Where(api.Equal(o2id, 2))
...
func TestNestedSetWithAdditionalJoin(t *testing.T) {
	...
	nestedJoinCond := api.And(
		api.Equal(o1rootid, o2rootid),
		api.Between(o1nestedleft, o2nestedleft, o2nestedright),
	)
	ouJoin := api.And(
		api.Equal(o2id, ouOrgId),
		api.Equal(ouUserId, 1),
	)
	q := api.Select(o1id)
...
func TestJoinDerivedWithMultipleSelections(t *testing.T) {
	...
	subq := api.Select(o1id).Join(o2, nestedJoinCond).Join(ou, ouJoin).As("derived")

	subqOrgId := subq.C("id")
	q := api.Select(subqOrgId)
